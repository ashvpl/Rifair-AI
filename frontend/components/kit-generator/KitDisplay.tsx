...
export function KitDisplay({ kit }: { kit: KitData }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Kit Header ──────────────────────────────────────────────── */}
      <div className="bg-gradient-to-br from-[#062c21] to-[#10b981] rounded-[1.75rem] p-6 md:p-8 text-white relative overflow-hidden shadow-xl">
        {/* Decorative blur */}
        ...
        <div className="flex items-start justify-between gap-4 relative z-10">
          <div className="space-y-2">
            ...
            <p className="text-sm text-emerald-200/90 leading-relaxed max-w-xl font-medium">
              {kit.kit_summary}
            </p>
          </div>
          <div className="text-right flex-shrink-0">
            <div className="text-3xl font-bold tabular-nums">
              {kit.estimated_duration_minutes || '—'}
            </div>
            <div className="text-[10px] font-medium text-emerald-300 uppercase tracking-wider">
              minutes
            </div>
          </div>
        </div>
      ...
      {/* Stats row — horizontal scroll on mobile */}
        <div className="flex gap-5 mt-5 pt-4 border-t border-white/10 relative z-10 overflow-x-auto no-scrollbar pb-1">
          {types.filter(t => t !== 'all').map((type, index) => (
            <div key={String(type) || `type-${index}`} className="shrink-0">
              ...
              <div className="text-[10px] font-medium text-emerald-300/70 uppercase tracking-wider capitalize whitespace-nowrap">
          ...
          <div className="ml-auto shrink-0">
            ...
            <div className="text-[10px] font-medium text-emerald-300/70 uppercase tracking-wider">
              Total
            </div>
          </div>
        </div>
      ...
      <div className="chip-row">
        {types.map((type, index) => (
          <button
            ...
            className={cn(
              "text-[11px] font-bold px-4 py-2 rounded-full border transition-all duration-200 capitalize whitespace-nowrap shrink-0 min-h-[36px]",
              activeFilter === type
                ? "bg-[#062c21] text-white border-[#062c21] shadow-md"
                : "bg-white text-[#86868B] border-black/[0.06] hover:border-[#062c21]/30 hover:bg-[#F5F5F7]"
            )}
          >
            {type === 'all'
      ...
      <div className="space-y-3">
        ...
        <div className="mt-4 text-center pb-4">
          <p className="text-xs text-neutral-400">
            ...
            {!isPaid && (
              <a href="/pricing" className="text-[#1D9E75] ml-1 font-medium">Upgrade</a>
            )}
          </p>
        </div>
      </div>
...
function KitActions({ 
  ...
  return (
    <div className="bg-white border border-black/[0.05] rounded-3xl p-8 text-center shadow-sm relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-md mx-auto space-y-4">
        ...
        <div className="pt-4">
          <button
            ...
            className="w-full bg-[#1e1b4b] text-white py-4 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-[#1a1640] shadow-lg shadow-[#1e1b4b]/20 hover:shadow-[#1e1b4b]/30 hover:-translate-y-0.5 transition-all"
          >
            Evaluate Candidate ↗
          </button>
        </div>
